...
async def upgrade(db: BaseDBAsyncClient) -> str:
    return """
        CREATE TABLE IF NOT EXISTS `b_static_recommended_perform` (
    ...
    CONSTRAINT `fk_b_user_p_b_static_642bc82d` FOREIGN KEY (`exam_standard_id`) REFERENCES `b_static_recommended_perform` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_b_user_p_b_user_4f9981e4` FOREIGN KEY (`user_id_id`) REFERENCES `b_user` (`id`) ON DELETE CASCADE
) CHARACTER SET utf8mb4;
        ALTER TABLE `b_user` ALTER COLUMN `username` SET DEFAULT '用户CFlb';"""
...
async def downgrade(db: BaseDBAsyncClient) -> str:
    return """
        ...
        DROP TABLE IF EXISTS `b_static_recommended_perform`;
